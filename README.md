...
### Стриминг ответов
- ✅ **Partial streaming** — edit-streaming: одно сообщение обновляется по мере генерации (как в Telegram)
- ✅ **Block streaming** — каждый блок текста отправляется отдельным сообщением
- ✅ **Настройка `streamMode`** — `off` (по умолчанию), `partial`, `block`
...
### Стриминг (`streamMode`)
- `off` — ответ отправляется целиком после генерации (по умолчанию)
- `partial` — сообщение отправляется сразу и редактируется по мере генерации (рекомендуется)
- `block` — каждый блок текста отправляется отдельным сообщением
